...
export default function ProviderRegisterX() {
  ...
  const [formData, setFormData] = useState({
   ...
   useEffect(() => {
    const registerStepCompleted = localStorage.getItem("registerStepCompleted");
    if (!registerStepCompleted) {  
      navigate('/register');
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.error) {
      ...
      localStorage.removeItem('registerStepCompleted')
    }
  };
    ...
    >
      ...
      <div className="relative bg-white rounded-lg shadow-xl flex flex-col md:flex-row max-w-5xl w-full overflow-hidden">
        ...
        <div className="bg-green-800 text-white flex flex-col items-center justify-center p-16 md:w-1/3 shadow-lg">
          ...
          <h1 className="text-4xl font-extrabold text-center">Welcome to Provider Registration</h1>
